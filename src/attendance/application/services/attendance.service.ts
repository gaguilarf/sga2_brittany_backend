...
export class AttendanceService {
    ...
    async create(createAttendanceDto: CreateAttendanceDto): Promise<AttendanceResponseDto> {
        try {
            this.logger.log(`Recording attendance for student ID: ${createAttendanceDto.studentId}`);
            const attendance = this.attendanceRepository.create(createAttendanceDto);
            const savedAttendance = await this.attendanceRepository.save(attendance);
            return this.toResponseDto(savedAttendance);
        } catch (error) {
            this.logger.error(`Error recording attendance: ${error.message}`, error.stack);
            throw error;
        }
    }
    ...
    async findAll(): Promise<AttendanceResponseDto[]> {
        try {
            ...
            this.logger.error(`Error fetching attendance: ${error.message}`, error.stack);
            throw error;
        }
    }
    ...
    async findOne(id: number): Promise<AttendanceResponseDto> {
        try {
            ...
            this.logger.error(`Error fetching attendance ${id}: ${error.message}`, error.stack);
            throw error;
        }
    }
    ...
    async update(id: number, updateAttendanceDto: UpdateAttendanceDto): Promise<AttendanceResponseDto> {
        try {
            ...
            this.logger.error(`Error updating attendance ${id}: ${error.message}`, error.stack);
            throw error;
        }
    }
    ...
    async remove(id: number): Promise<void> {
        try {
            ...
            this.logger.error(`Error removing attendance ${id}: ${error.message}`, error.stack);
            throw error;
        }
    }

    private toResponseDto(entity: AttendanceTypeOrmEntity): AttendanceResponseDto {
        return {
            id: entity.id,
