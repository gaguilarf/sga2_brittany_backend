...
export class CampusesService {
    ...
    async create(createCampusDto: CreateCampusDto): Promise<CampusResponseDto> {
        try {
            ...
            this.logger.log(`Campus created successfully: ${savedCampus.name} (ID: ${savedCampus.id})`);
    ...
    async findAll(): Promise<CampusResponseDto[]> {
        try {
            ...
            this.logger.error(`Error fetching campuses: ${error.message}`, error.stack);
            throw error;
        }
    }
    ...
    async findActive(): Promise<CampusResponseDto[]> {
        try {
            ...
            this.logger.error(`Error fetching active campuses: ${error.message}`, error.stack);
            throw error;
        }
    }
    ...
    async findOne(id: number): Promise<CampusResponseDto> {
        try {
            ...
            this.logger.error(`Error fetching campus ${id}: ${error.message}`, error.stack);
            throw error;
        }
    }

    async update(id: number, updateCampusDto: UpdateCampusDto): Promise<CampusResponseDto> {
        try {
            ...
            this.logger.log(`Campus updated successfully: ${updatedCampus.name} (ID: ${updatedCampus.id})`);

            return this.toResponseDto(updatedCampus);
        } catch (error) {
            this.logger.error(`Error updating campus ${id}: ${error.message}`, error.stack);
            throw error;
        }
    }
    ...
    async remove(id: number): Promise<void> {
        try {
            ...
            this.logger.log(`Campus removed successfully: ${campus.name} (ID: ${id})`);
        } catch (error) {
            this.logger.error(`Error removing campus ${id}: ${error.message}`, error.stack);
            throw error;
        }
    }
