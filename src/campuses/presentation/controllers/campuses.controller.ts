...
export class CampusesController {
    ...
    async findOne(@Param('id', ParseIntPipe) id: number): Promise<CampusResponseDto> {
    ...
    @ApiResponse({ status: 403, description: 'Forbidden - Insufficient permissions' })
    @ApiResponse({ status: 404, description: 'Campus not found' })
    @ApiResponse({ status: 409, description: 'Conflict - Campus name already exists' })
    async update(
    ...
    @ApiResponse({ status: 401, description: 'Unauthorized' })
    @ApiResponse({ status: 403, description: 'Forbidden - Only administrators can delete' })
    @ApiResponse({ status: 404, description: 'Campus not found' })
