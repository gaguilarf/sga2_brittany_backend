...
export class StudentsService {
    ...
    async create(createStudentDto: CreateStudentDto): Promise<StudentResponseDto> {
        try {
            ...
            if (createStudentDto.dni) {
                ...
                if (existingStudent) {
                    throw new ConflictException(`Student with DNI "${createStudentDto.dni}" already exists`);
                }
            }
            ...
            this.logger.log(`Student created successfully: ${savedStudent.nombre} (ID: ${savedStudent.id})`);
    ...
    async findActive(): Promise<StudentResponseDto[]> {
        try {
            ...
            this.logger.error(`Error fetching active students: ${error.message}`, error.stack);
            throw error;
        }
    }
    ...
    async findOne(id: number): Promise<StudentResponseDto> {
        try {
            ...
            this.logger.error(`Error fetching student ${id}: ${error.message}`, error.stack);
            throw error;
        }
    }
    ...
    async update(id: number, updateStudentDto: UpdateStudentDto): Promise<StudentResponseDto> {
        try {
            ...
            if (updateStudentDto.dni && updateStudentDto.dni !== student.dni) {
                ...
                if (existingStudent) {
                    throw new ConflictException(`Student with DNI "${updateStudentDto.dni}" already exists`);
                }
            }
            ...
            this.logger.log(`Student updated successfully: ${updatedStudent.nombre} (ID: ${updatedStudent.id})`);

            return this.toResponseDto(updatedStudent);
        } catch (error) {
            this.logger.error(`Error updating student ${id}: ${error.message}`, error.stack);
            throw error;
        }
    }
    ...
    async remove(id: number): Promise<void> {
        try {
            ...
            this.logger.log(`Student removed successfully: ${student.nombre} (ID: ${id})`);
        } catch (error) {
            this.logger.error(`Error removing student ${id}: ${error.message}`, error.stack);
            throw error;
        }
    }
