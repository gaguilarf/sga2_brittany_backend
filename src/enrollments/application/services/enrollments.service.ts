...
export class EnrollmentsService {
    ...
    async create(createEnrollmentDto: CreateEnrollmentDto): Promise<EnrollmentResponseDto> {
        try {
            this.logger.log(`Creating new enrollment for student ID: ${createEnrollmentDto.studentId}`);
            ...
            this.logger.log(`Enrollment created successfully with ID: ${savedEnrollment.id}`);
    ...
    async update(id: number, updateEnrollmentDto: UpdateEnrollmentDto): Promise<EnrollmentResponseDto> {
        try {
            ...
            const updatedEnrollment = await this.enrollmentsRepository.save(enrollment);

            this.logger.log(`Enrollment updated successfully: ID ${updatedEnrollment.id}`);
            return this.toResponseDto(updatedEnrollment);
        } catch (error) {
            this.logger.error(`Error updating enrollment ${id}: ${error.message}`, error.stack);
            throw error;
        }
    }
    ...
    async remove(id: number): Promise<void> {
        try {
            ...
            this.logger.error(`Error removing enrollment ${id}: ${error.message}`, error.stack);
            throw error;
        }
    }

    private toResponseDto(entity: EnrollmentsTypeOrmEntity): EnrollmentResponseDto {
        return {
            id: entity.id,
