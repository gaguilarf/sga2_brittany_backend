...
export class GradesService {
    ...
    async create(createGradeDto: CreateGradeDto): Promise<GradeResponseDto> {
        try {
            this.logger.log(`Creating new grade for student ID: ${createGradeDto.studentId}`);
    ...
    async findOne(id: number): Promise<GradeResponseDto> {
        try {
            ...
            this.logger.error(`Error fetching grade ${id}: ${error.message}`, error.stack);
            throw error;
        }
    }
    ...
    async update(id: number, updateGradeDto: UpdateGradeDto): Promise<GradeResponseDto> {
        try {
            ...
            this.logger.error(`Error updating grade ${id}: ${error.message}`, error.stack);
            throw error;
        }
    }
    ...
    async remove(id: number): Promise<void> {
        try {
            ...
            this.logger.error(`Error removing grade ${id}: ${error.message}`, error.stack);
            throw error;
        }
    }

    // Grade Details
    async addDetail(createDetailDto: CreateGradeDetailDto): Promise<GradeDetailResponseDto> {
        try {
            ...
            if (grade) {
                const totalScore = grade.details.reduce((acc, curr) => acc + Number(curr.puntaje), 0);
            ...
            this.logger.error(`Error adding grade detail: ${error.message}`, error.stack);
            throw error;
        }
    }
    ...
    private toDetailResponseDto(entity: GradeDetailsTypeOrmEntity): GradeDetailResponseDto {
        return {
            id: entity.id,
